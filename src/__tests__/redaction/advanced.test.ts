...
describe('Data Redaction - Advanced Features', () => {
  ...
  describe('Advanced Integration Tests', () => {
    ...
    test('should maintain performance with large datasets', () => {
      // Comprehensive CI/constrained environment detection
      const isCI = !!(
        process.env.CI ||
        ...
        process.env.JEST_WORKER_ID // Jest parallel workers
      );
      ...
      // Always test functional correctness first
      const result = DataRedactor.redactData(largeData);

      // Verify redaction worked correctly regardless of environment
      ...
      // Only run performance assertions in local development environments
      if (!isCI) {
        ...
        // Re-test large dataset for performance measurement
        const startTime = performance.now();
        DataRedactor.redactData(largeData);
        const endTime = performance.now();
        const executionTime = endTime - startTime;

        // Performance should scale reasonably (not more than 15x the baseline for 10x data)
        // Use more conservative scaling factor and add minimum threshold
        const maxScalingFactor = 15;
        const minimumThreshold = 100; // 100ms minimum threshold
        const expectedMaxTime = Math.max(baselineTime * maxScalingFactor, minimumThreshold);

        expect(executionTime).toBeLessThan(expectedMaxTime);

        // Log performance metrics for debugging (only in non-CI)
        if (process.env.DEBUG_PERFORMANCE) {
          console.log(`Baseline (100 items): ${baselineTime.toFixed(2)}ms`);
          console.log(`Large dataset (1000 items): ${executionTime.toFixed(2)}ms`);
  ...
  describe('Circular reference and edge cases', () => {
    ...
    test('should handle maximum depth gracefully', () => {
      const deepObj: any = { level: 0, password: 'secret' };
      let current = deepObj;
